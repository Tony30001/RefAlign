...
class WanVideoPipeline(BasePipeline):
    ...
    def training_loss(self, **inputs):
        ...
        # print("time", timestep,"gate",gate_for_dino)
        ...
        loss_mse = loss
        align_loss1 = align_loss1*gate_for_dino
        # loss = loss + gate_for_dino*0.01*align_loss1+ 1.0*align_loss2
        # loss = losss
        # print(qwen_loss)
        # print(timestep, gate_for_dino)
        return loss_mse,align_loss1, gate_for_dino*align_loss2
    ...
    def from_pretrained(
        ...
        tokenizer_config: ModelConfig = ModelConfig(model_id="models/Wan2.1-T2V-14B", origin_file_pattern="google/*"),
        local_model_path: str = ".../DiffSynthStudio/models",
        skip_download: bool = True,
        redirect_common_files: bool = True,
        use_usp=False,
        dinov3_model_id: str = ".../dinov3_weights_l",
    ):
        # Redirect model path
    ...
    def __call__(
        ...
        #####hook
        # 存储：attn_hiddens[step_idx][layer_idx] = Tensor(B, N, D) on CPU
        attn_hiddens = {}
        _hook_handles = []

        # 你可以只抓某些层/某些step，避免爆内存
        hook_layers = None       # 例如 [0, 1, 2]；None 表示抓全部层
        capture_steps = {0}      # 例如只抓第0个timestep；想全抓就设 None

        # # 用闭包拿到 layer_id
        # def make_attn_hook(layer_id: int):
        #     def hook_fn(module, inputs, output):
        #         # output 可能是 Tensor，也可能是 (Tensor, ...)；只取主输出
        #         out = output[0] if _is_tuple(output) else output
        #         # out 期望形状 (B, N, D) 或类似
        #         step = hook_state["step"]
        #         if (capture_steps is not None) and (step not in capture_steps):
        #             return

        #         # 只存指定层
        #         if (hook_layers is not None) and (layer_id not in hook_layers):
        #             return

        #         # 存CPU float32，减轻显存
        #         branch = hook_state["branch"]  # "pos_it" / "pos_i" / "neg"
        #         attn_hiddens.setdefault(step, {}).setdefault(branch, {})[layer_id] = out.detach().float().cpu()
        #     return hook_fn

        # # 记录当前step给hook用
        # hook_state = {"step": -1, "branch": "unknown"}


        # # 注册到每个 block 的 self_attn
        # for i, block in enumerate(self.model.blocks):
        #     h = block.self_attn.register_forward_hook(make_attn_hook(i))
        ...
        for progress_id, timestep in enumerate(progress_bar_cmd(self.scheduler.timesteps)):
            ...
            step_ratio = progress_id / (num_steps - 1)
            ...
            # print(inputs_shared["subject_latents"].shape)
            # assert 2==1
            ...
            if cfg_scale != 1.0:
                if cfg_merge:
                    ...
                    neg_inputs["subject_latents"] = neg_inputs["subject_latents"] * 0.0
...
class WanVideoUnit_Subject(PipelineUnit):
    ...
    def process(self, pipe: WanVideoPipeline, subject_image, height, width):
        if subject_image is None:
            reference_latents = torch.zeros((1, 16, 1, 60, 104), device=pipe.device, dtype=pipe.torch_dtype)
            ref_qwen_vis = None
            use_dino = bool(getattr(pipe.scheduler, "training", False) and pipe.training)
            if use_dino:
                ref_qwen_vis = torch.zeros((1, 1024, 1, 30, 52), device=pipe.device, dtype=pipe.torch_dtype)
        ...
        for image in images:
            ...
            if use_dino:
                ...
                with torch.inference_mode():
                    ...
                    hidden_states = outputs.hidden_states[-1][:, 5:].view(1, 30, 52, 1024).permute(0, 3, 1, 2).contiguous().unsqueeze(2)
...
def compute_semantic_alignment_loss(ref_qwen_latents, y_ip):
    ...
    loss = (1-(q * s)).sum(dim=-1).mean() 
    # print(f"loss_cos: {loss_cos:.4f}")

    return loss

def cosine_margin_loss(student, teacher, margin=0.2):
    """
    student/teacher: (B, L, C)
    base = 1 - cos in [0, 2]
    Loss: max(0, (1 - cos) - margin)
    Meaning: only push closer if distance > margin; stop once within margin.
    """
    s = F.normalize(student, dim=-1)
    t = F.normalize(teacher, dim=-1)
    cos = (s * t).sum(dim=-1)          # (B, L)
    dist = 1.0 - cos                   # (B, L)
    return F.relu(dist - margin).mean()
...
# RA loss
def cosine_margin_pos_inter_subject_neg_flat(
    student: torch.Tensor,   # (BL, C)  你这里 B=1 -> (L,C)
    teacher: torch.Tensor,   # (BL, C)
    tokens_per_subject: int = 1560,
    pos_margin: float = 0.0,
    neg_margin: float = 1.0,
    neg_weight: float = 1.0,
):
    """
    输入：student/teacher = (N, C)，N = B*L（你现在 B=1，因此 N=L）
    正：逐 token 对齐（i 对 i）
      dist_pos[i] = 1 - cos(s_i, t_i)
      pos_loss = mean(relu(dist_pos - pos_margin))

    负：仅主体间（按 tokens_per_subject 分块）
      dist_neg[i,j] = 1 - cos(s_i, t_j),  block(i)!=block(j)
      neg_loss = mean(relu(neg_margin - dist_neg))  (只在跨主体 pairs 上)
    """
    assert student.dim() == 2 and teacher.dim() == 2
    assert teacher.shape == student.shape
    N, C = student.shape
    T = tokens_per_subject
    assert N % T == 0, f"N={N} must be multiple of tokens_per_subject={T}"
    K = N // T

    s = F.normalize(student, dim=-1)  # (N,C)
    t = F.normalize(teacher, dim=-1)  # (N,C)

    # --- 正：逐 token（对角线 i==i） ---
    cos_pos = (s * t).sum(dim=-1)      # (N,)
    dist_pos = 1.0 - cos_pos           # (N,)
    pos_loss = F.relu(dist_pos - pos_margin).mean()

    # --- 负：主体间 ---
    if K <= 1:
        neg_loss = student.new_tensor(0.0)
        total = pos_loss
        return total, pos_loss, neg_loss

    # (N,N) 距离矩阵
    cos_mat = s @ t.t()                # (N,N)
    dist_mat = 1.0 - cos_mat           # (N,N)

    idx = torch.arange(N, device=student.device)
    block = idx // T                   # (N,)

    neg_mask = block.view(N, 1).ne(block.view(1, N))  # 跨主体
    neg_mask.fill_diagonal_(False)                    # 排掉 i==i（稳妥）

    dist_neg = dist_mat[neg_mask]      # (num_neg,)
    neg_loss = F.relu(neg_margin - dist_neg).mean()

    total = pos_loss + neg_weight * neg_loss
    return total, pos_loss, neg_loss
    ...
    **kwargs,
    ...
    if subject_latents is not None:
        ...
        if ref_qwen_latents is not None:
            flag = True
            null_subject = (ref_qwen_latents.abs().sum() == 0)
            # if null_subject:
                # print("置空成功")
        ...
        if use_input_mlp:
            ...
            if flag:
                ...
                # ref_qwen_latents = ref_qwen_latents.mean(dim=0, keepdim=True)
            # if ref_qwen_latents is not None:
                # p = 0.1  # 10% 概率
                # if random.random() < p:
                #     x_ip = x_ip*0
            ...
            #             torch.cat(
            #                 [
            #                     dit.freqs[0][f:f+f_ip].view(f_ip, 1, 1, -1).expand(f_ip, h_ip, w_ip, -1),
            #                     dit.freqs[1][ h: h+h_ip]
            #                     .view(1, h_ip, 1, -1)
            #                     .expand(f_ip, h_ip, w_ip, -1),
            #                     dit.freqs[2][w:w+w_ip]
            #                     .view(1, 1, w_ip, -1)
            #                     .expand(f_ip, h_ip, w_ip, -1),
            #                 ],
            #                 dim=-1,
    ...
    if tea_cache_update:
            ...
            if use_gradient_checkpointing_offload:
                with torch.autograd.graph.save_on_cpu():
                    x,x_ip = torch.utils.checkpoint.checkpoint(
                        ...
                        x, context, t_mod, this_freqs,x_ip, cond_size, image_num,this_t_mod_ip,timestep,decoubed_subject,
                        use_reentrant=False,
                    )
                ...
                x,x_ip = block(x, context, t_mod, this_freqs, x_ip, cond_size, image_num,this_t_mod_ip,timestep,decoubed_subject)
            if block_id < 9 and block_id >=0 and flag:
                # x_ip = x_ip.mean(dim=1, keepdim=True)
                x_ip_proj = rearrange(x_ip, 'b l c -> (b l) c').contiguous()
                x_ip_proj = block.input_mlp(x_ip_proj)
                loss_align_layer_mean,_,neg_loss = cosine_margin_pos_inter_subject_neg_flat(x_ip_proj, ref_qwen_latents)
                if null_subject:
                    loss_align_layer_mean = loss_align_layer_mean*0
                    neg_loss = neg_loss*0
                align_loss.append(loss_align_layer_mean)
                align_loss2.append(neg_loss)
                # x_ip = res_vae
                # x_ipy_ip
        ...
        if flag:
            ...
            align_loss2 = torch.stack(align_loss2).mean()
    ...
    # else:
    if flag:
        return x, align_loss, align_loss2
    return x,0,0
